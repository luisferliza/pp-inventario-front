...
import { EncargadoEdicionComponent } from './encargado-edicion/encargado-edicion.component';


@Component({
  selector: 'elastic-encargado',
  templateUrl: './encargado.component.html',
  styleUrls: ['./encargado.component.scss'],
  animations: [...ROUTE_TRANSITION],
  host: { '[@routeTransition]': '' }
})
export class EncargadoComponent implements List<Articulo>, OnInit, OnDestroy {
  ...
  @Input()
  columns: ListColumn[] = [
    { name: 'ID_Encargado', property: 'p1', visible: false, isModelProperty: true },
  ...
  crear() {        
    this.dialog.open(EncargadoEdicionComponent).afterClosed().subscribe((articulo: Articulo) => {
      if (articulo) {
        this.listar();
  ...
  modificar(estado) {    
    this.dialog.open(EncargadoEdicionComponent, {
      data: estado
