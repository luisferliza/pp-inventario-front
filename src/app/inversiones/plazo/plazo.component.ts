...
import { PlazoEdicionComponent } from './plazo-edicion/plazo-edicion.component';

@Component({
  selector: 'elastic-plazo',
  templateUrl: './plazo.component.html',
  styleUrls: ['./plazo.component.scss'],
  animations: [...ROUTE_TRANSITION],
  host: { '[@routeTransition]': '' }
})
export class PlazoComponent implements List<Articulo>, OnInit, OnDestroy {
  ...
  @Input()
  columns: ListColumn[] = [
    { name: 'ID_Plazo', property: 'p1', visible: false, isModelProperty: true },
  ...
  crear() {        
    this.dialog.open(PlazoEdicionComponent).afterClosed().subscribe((articulo: Articulo) => {
      if (articulo) {
        this.listar();
  ...
  modificar(estado) {    
    this.dialog.open(PlazoEdicionComponent, {
      data: estado
